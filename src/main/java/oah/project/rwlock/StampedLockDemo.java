...
public class StampedLockDemo {
    ...
    public void tryOptimisticRead() {
        ...
        System.out.println("4秒前stampedLock.validate()方法值(true无法修改，false有修改)" + "\t" + stampedLock.validate(stamp));
        for (int i = 1; i <= 4; i++) {
            try {
                TimeUnit.SECONDS.sleep(1);
            } catch (InterruptedException e) {
            ...
            System.out.println(Thread.currentThread().getName() + "\t" + "正在读取......  " + i + "  秒" +
                    "后stampedLock.validate()方法值(true无修改，false有修改)" + "\t" + stampedLock.validate(stamp));
        }
